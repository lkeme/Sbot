...
__all__ = ["T_BaseMessage", "T_BaseMessageSegment", "T_BaseMS", "BaseMessage", "BaseMessageSegment"]

T_BaseMessage = TypeVar("T_BaseMessage", bound="BaseMessage")
T_BaseMessageSegment = TypeVar("T_BaseMessageSegment", bound="BaseMessageSegment")

# 可以转化为 MessageSegment 的类型
T_BaseMS = Union[T_BaseMessageSegment, str, Mapping]


class BaseMessage(List[T_BaseMessageSegment]):
    """消息。
    本类是 `List` 的子类，并重写了 `__init__()` 方法，
    可以直接处理 str, Mapping, Iterable[Mapping], T_BaseMessageSegment, T_BaseMessage。
    ...
    def __init__(
            self,
            message: Union[None, T_BaseMessage, T_BaseMS, Iterable[T_BaseMS]] = None,
            *args,
            **kwargs,
    ):
    ...
    @property
    def _message_segment_class(self) -> Type[T_BaseMessageSegment]:
        """若开发者实现了 MessageSegment 的子类则需要重写此方法。
        Returns:
            MessageSegment 类。
        """
        return BaseMessageSegment
    ...
    def _construct(
            self, msg: Union[T_BaseMS, Iterable[T_BaseMS]]
    ) -> Iterator[T_BaseMessageSegment]:
    ...
    def _mapping_to_message_segment(self, msg: Mapping) -> T_BaseMessageSegment:
    ...
    def _str_to_message_segment(self, msg: str) -> T_BaseMessageSegment:
    ...
    def __add__(
            self: T_BaseMessage, other: Union[T_BaseMessage, T_BaseMessageSegment, str]
    ) -> T_BaseMessage:
    ...
    def deepcopy(self) -> T_BaseMessage:
        """返回自身的深复制。
        Returns:
            自身的深复制。
        """
    ...
    def startswith(
            self, prefix: Union[str, T_BaseMessageSegment], start=None, end=None
    ) -> bool:
        """实现类似字符串的 `startswith()` 方法。
        当 `prefix` 类型是 str 时，会将自身转换为 str 类型，再调用 str 类型的 `startswith()` 方法。
        当 `prefix` 类型是 T_BaseMessageSegment 时，`start` 和 `end` 参数将不其作用，
    ...
    def endswith(
            self, suffix: Union[str, T_BaseMessageSegment], start=None, end=None
    ) -> bool:
        """实现类似字符串的 `endswith()` 方法。
        当 `suffix` 类型是 str 时，会将自身转换为 str 类型，再调用 str 类型的 `endswith()` 方法。
        当 `suffix` 类型是 T_BaseMessageSegment 时，`start` 和 `end` 参数将不其作用，
    ...
    def replace(
            self,
            old: Union[str, T_BaseMessageSegment],
            new: Union[str, T_BaseMessageSegment, None],
            count: int = -1,
    ) -> T_BaseMessage:
        """实现类似字符串的 `replace()` 方法。
    ...
    def _replace_str(self, old: str, new: str, count: int = -1) -> T_BaseMessage:
        """实现类似字符串的 `replace()` 方法。
...
@dataclass
class BaseMessageSegment(Mapping, Generic[T_BaseMessage]):
    """消息字段。
    ...
    @property
    def _message_class(self) -> Type[T_BaseMessage]:
    ...
    def __eq__(self, other: T_BaseMessageSegment) -> bool:
        return self.type == other.type and self.data == other.data

    def __ne__(self, other: T_BaseMessageSegment) -> bool:
        return not self.__eq__(other)

    def __add__(self, other) -> T_BaseMessage:
        return self._message_class(self) + other

    def __radd__(self, other) -> T_BaseMessage:
    ...
    def copy(self) -> T_BaseMessageSegment:
        """返回自身的浅复制。
        Returns:
            自身的浅复制。
        """
    ...
    def deepcopy(self) -> T_BaseMessageSegment:
        """返回自身的深复制。
        Returns:
            自身的深复制。
        """
